...
# Force register JavaScript MIME type before any other operations
...
<p class="status">API Version 1.0.0 &bull; <a href="/api/health">/api/health</a></p>
    ...
    if has_frontend:
        print(f"DEBUG: has_frontend=True, static_dir={static_dir}")
        # SPA 路由回退
        @app.get("/{full_path:path}", include_in_schema=False)
        async def serve_spa(request: Request, full_path: str):
            """SPA 路由回退 - 非 API 路由返回 index.html"""
            print(f"DEBUG: serve_spa called with path={full_path}")
            ...
            logger.warning(f"DEBUG: Request path: {full_path}, File exists: {file_path.exists()}, Is file: {file_path.is_file()}")
            if file_path.exists() and file_path.is_file():
                # Issue #520: Explicitly resolve MIME type to avoid
                # browsers rejecting JS modules served as text/plain.
                # Windows may return text/plain for .js files, so we force correct types
                suffix = file_path.suffix.lower()
                logger.warning(f"DEBUG: Suffix: {suffix}")
                if suffix == '.js':
